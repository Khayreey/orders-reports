...
const ExpandPendingOrders = ({ record }: ExpandrecordListInterface) => {
  ...
  const typesIds = record.products.map((e: any) => {
    if ( e.product && e.product?.type && e.product?.type !== "") return e.product.type._id;
  });
...
/// get ships and products and getProducts after update to display the new quantity
  ...
  return (
    <>
    <AcceptModal details={details} setDetails={setDetails} price={price} setPrice={setPrice} formattedProducts={formattedProducts} setProducts={setProducts} deleteProduct={deleteProduct} products={products} isModalOpen={isModalOpen} handleCancel={handleCancel} handleOk={handleUpdateDeliver}/>
    <Formik
      initialValues={{
        name: record.name,
        phone: record.phone,
          ...
          <Form onSubmitCapture={handleSubmit} layout="vertical" ref={formRef}>
           ...
           null
            ...
            <CustomVarSelectFormik
              placeholder="الحالة"
              label="الحالة"
              options={record.status === 'قيد التشغيل' ? statusData : statusPendingData}
              name="status"
            />
            <Button
              type="primary"
